...
import { isIndexedDbAvailable, saveReactionClipToDb } from "@/lib/db";
import { emitReactionClipSaved } from "@/lib/events";
...
export default function ReactionRecorder() {
    ...
    useEffect(() => {
        ...
        const mime = hasMediaRecorder ? getSupportedMimeType() : "non supportato";

        setBrowserStatus(
            `MediaRecorder: ${hasMediaRecorder ? "ok" : "non supportato"} • IndexedDB: ${hasIndexedDb ? "ok" : "non supportato"
            } • Codec: ${mime}`
        );
    ...
    const setupRecorder = async () => {
        ...
        if (typeof MediaRecorder === "undefined") {
            setErrorText("Questo browser non supporta MediaRecorder.");
            showToast("Browser non supportato per la registrazione video.", "error");
        ...
        try {
            ...
            showToast("Camera e microfono attivati.", "success");
            monitorAudio();
        } catch (err) {
            console.error(err);
            setRecorderState("error");
            setErrorText("Non riesco ad accedere a camera o microfono.");
            showToast("Permessi camera/microfono negati o non disponibili.", "error");
        }
    };
    ...
    const startRecording = () => {
        ...
        try {
            ...
            recorder.onstop = async () => {
                try {
                    ...
                    if (isIndexedDbAvailable()) {
                        ...
                        emitReactionClipSaved();
                        setSavedMessage("Clip salvata nella gallery locale.");
                        showToast("Clip salvata con successo.", "success");
                    } else {
                        showToast("Clip registrata, ma il salvataggio persistente non è supportato qui.", "info");
                    }
                } catch (error) {
                    console.error(error);
                    showToast("Registrazione completata, ma salvataggio fallito.", "error");
                }
            };
            ...
            console.error(error);
            setRecorderState("error");
            setErrorText("Non sono riuscito ad avviare la registrazione.");
            showToast("Avvio registrazione non riuscito.", "error");
        }
    };
    ...
    const stopRecording = () => {
        ...
        if (mediaRecorderRef.current.state === "recording") {
            ...
            showToast("Sto processando il video...", "info");
        }
    };
    ...
    const resetRecorder = () => {
        ...
        showToast("Recorder resettato.", "info");
    };
        ...
        <>
            <div className="card">
                <div className="galleryHeader">
                    <div>
                        <h2 style={{ marginBottom: 6 }}>📹 Reaction Recorder V6.1</h2>
                        <p className="small">
                            Recorder verticale con overlay dinamico, caption automatica, salvataggio persistente e UX più robusta.
                        </p>
                    </div>
                </div>

                <div className="note" style={{ marginTop: 12 }}>
                    Stato browser: <b>{browserStatus}</b>
                </div>
                ...
                {savedMessage && <div className="note">{savedMessage}</div>}
